...
class CheckoutProductController {
  ...
  static async getAllCheckoutProducts(req, res, next) {
    try {
      ...
      for (const checkout of checkouts) {
        ...
        //  checkout products with quantity in the map
  ...
  static async addCheckoutProduct(req, res, next) {
    ...
    try {
      ...
      let idTemp;
      if (!findedCheckout) {
        let cretedCheckout = await Checkout.create(
          { userId: req.user.id },
          { transaction: t }
        );
      ...
      if (checkCheckoutProduct) {
        await checkCheckoutProduct.increment("quantity", {
          by: 1,
          transaction: t,
        });
      } else {
  ...
  static async detailsCheckoutProduct(req, res, next) {
    try {
      ...
      if (checkoutProducts.length === 0) {
        return res
          .status(404)
          .json({ message: "Checkout products not found." });
      }
